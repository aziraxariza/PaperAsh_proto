...
```bash
git clone https://github.com/aziraxariza/paper-ash-diary.git
cd paper-ash-diary
```
...
Configure the database, authentication, and storage credentials.
...
The application runs on the configured local development port.
...
Create the owner account/password hash using:

```bash
pnpm create-admin
```

The resulting credentials are configured through environment variables rather than being stored in source code.
...
```bash
docker build -t paper-ash-diary .
```

Run the container:

```bash
docker run --env-file .env -p 3000:3000 paper-ash-diary
```
...
The repository includes deployment configuration for Render and container-based environments.
